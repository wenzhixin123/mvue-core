<template>
    <div v-if="preprocessed" class="meta-form-panel" :class="{'has-buttons':hasButtons()}">
            ...
            <div v-for="formItem in metaForm.layout" :key="formItem.id">
                <div class="control-tmpl-panel" v-show="!formItem.hidden">
                    <component v-if="formItem.isContainer" :is="'Meta'+formItem.componentType" :form-item="formItem">
                        <div v-for="containerFormItem in formItem.children" :key="containerFormItem.id"
                             v-show="!containerFormItem.hidden">
                            <component v-if="containerFormItem.isDataField" :context="fieldContext(formItem)"
                                       :validator="$validator" v-model="entity[containerFormItem.dataField]"
                                       @exDataChanged="exDataChanged" :is="componentName(formItem)"
                                       :form-item="containerFormItem" :paths="paths" :model="entity"></component>
                            <component v-else-if="containerFormItem.isExternal" :context="fieldContext(formItem)"
                                       @on-register-after-save-chain="registerAfterSaveChain"
                                       :is="componentName(containerFormItem)" :form-item="containerFormItem"
                                       :paths="paths" :model="entity"></component>
                            <component v-else :is="'Meta'+containerFormItem.componentType"
                                       :form-item="containerFormItem"></component>
                        </div>
                    </component>
                    <component v-else-if="formItem.isDataField" :context="fieldContext(formItem)"
                               :validator="$validator" v-model="entity[formItem.dataField]"
                               @exDataChanged="exDataChanged" :is="componentName(formItem)" :form-item="formItem"
                               :paths="paths" :model="entity"></component>
                    <component v-else-if="formItem.isExternal" :context="fieldContext(formItem)"
                               @on-register-after-save-chain="registerAfterSaveChain" :is="componentName(formItem)"
                               :form-item="formItem" :paths="paths" :model="entity"></component>
        ...
        <div v-transfer-dom="'#default-form-uuid-'+entityName" :data-transfer="transfer" class="form-toolbar"
             :class="{'has-buttons':hasButtons()}" slot="toolbar">
            <div v-if="hasButtons()" :class="{'onepx-stroke':hasButtons()}"></div>
            <template v-if="toolbar">
                <template v-if="isView">
                    <meta-operation v-for="btn in toolbar.viewBtns" :key="btn.name" :operation="btn"
                                    :widget-context="getWidgetContext()" @triggered="operationTriggered">
                ...
                <template v-if="!isView">
                    <meta-operation v-for="btn in toolbar.editBtns" :key="btn.name" :operation="btn"
                                    :widget-context="getWidgetContext()" @triggered="operationTriggered">
                        <Button :type="btn.btnType || 'primary'" size="small" :title="btn.title">
                            <Icon :type="btn.icon" v-if="btn.icon"></Icon>
                            {{btn.title}}
