<template>
    <div class="grid-import-data-con">
        <div @click="showImport">
            <slot>
                <Button v-if="operation.toolbarType=='compact'" type="text" :icon="operation.icon">{{operation.title}}
                </Button>
            ...
            <div id="import-data-scroll-con" style="height:340px;overflow:auto;padding:4px;">
                ...
                <div v-show="current===0">
                    ...
                    <div class="upload-con margin-top42">
                        <Row>
                            <Col span="24">
                                <import-file-upload :validator="$validator"
                                                    v-model="model.file"
                                                    :form-item="excelUploadFormItem"
                                                    :paths="paths"></import-file-upload>
                                <div style="margin-left:auto;margin-right:auto;width:220px;margin-top:10px;">
                                    ...
                                    -->
                                    <a v-if="templateUrl.length>0"
                                       style="margin-left: 36px;"
                                       :href="templateUrl">下载模板</a>
                                    <Alert v-if="templateUrl.length<=0" type="warning" show-icon>管理员未设置模板</Alert>
                                </div>
                            </Col>
                        </Row>
                    </div>
                    <div class="col-md-10 col-md-offset-1 font12 tips-color" style="margin-top:24px;">
                        <div class="divider_buttom" v-if="canCover"
                             style="margin-top: 16px; margin-bottom: 10px; padding-bottom: 6px">
                            <Checkbox label="twitter" @on-change="onCheckChanged">
                                <span class="textContent">是否覆盖已有数据</span>
                            </Checkbox>
                        </div>
                        <div>注意事项</div>
                        <div>
                ...
                <div v-show="current===1">
                    <table class="table field-mapping-con"
                           v-if="modelForImport.mappings&&modelForImport.mappings.length">
                        <thead>
                        <tr>
                            <th>序号</th>
                            <th>Excel列名</th>
                        ...
                        <tr v-for="(mapping,index) in modelForImport.mappings" :key="index">
                            ...
                            <td>
                                <select v-model="mapping.fieldName">
                                    <option v-for="field in metaEntity.fields" :value="field.name" :key="field.name">
                                        {{field.title}}
                                    </option>
                                </select>
                            </td>
                            <td><input type="checkbox" v-model="mapping.nameToId" :true-value="true"
                                       :false-value="false"></td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                    ...
                    <div class="report-con margin-top42" v-if="report">
                        <Row v-for="(sr,index) in report.summaryReport" :key="index">
                            <Col span="24" class="textalignL">
                                已成功创建<span class="highlight-number">{{sr.created}}</span>个/更新<span
                                    class="highlight-number">{{sr.updated}}</span>个[{{sr.entityDisplayName}}]记录 失败<span
                                    class="highlight-number">{{sr.error}}</span>个
                            </Col>
                        </Row>
...
<script>
  ...
  export default {
    ...
    data: function () {
      ...
      var metaEntity = this.widgetContext.metaEntity
      if (typeof metaEntity == 'string') {
        metaEntity = JSON.parse(metaEntity)
      }
      ...
      let metaEntityId = metaEntity.metaEntityId
      return {
        grid: grid,
